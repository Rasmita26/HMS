<?php
    ...
    if(isset($_POST['data']) && isset($_POST['data1']) ){
        ...
        // $created_by=get_object_vars($data1)['created_by'];
        $pono=mysqli_fetch_assoc(mysqli_query($con,"SELECT max(pono) x FROM purchase ORDER BY id DESC "))['x'];
        if (!$pono) {
          $pono='PO/01';       
        }else{
          $pono++;
        ...
        foreach($data as $i){
          ...
          // echo "INSERT INTO purchase (pono,purchasedate,supplierid,medicineid,unit,quantity,rate,actual_amount,descper,descamt,amount_afterdesc,cgstper,cgstamt,sgstper,sgstamt,igstper,igstamt,netamount,created_by,created_time) VALUES
